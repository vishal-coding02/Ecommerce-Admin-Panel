...
async function loginForm(req, res) {
    const user = await UserModel_1.default.findOne({ userEmail: req.body.email });
    if (!user) {
...
async function updateStatus(req, res) {
    ...
    try {
        if (req.user.role === "admin") {
            ...
            const updatedUser = await UserModel_1.default.findByIdAndUpdate(req.body.id, {
...
async function deleteUser(req, res) {
    ...
    try {
        if (req.user.role === "admin") {
            await UserModel_1.default.findByIdAndDelete(req.body.id);
...
async function logout(req, res) {
    ...
    try {
        const user = await UserModel_1.default.findOne({ _id: req.body.id });
